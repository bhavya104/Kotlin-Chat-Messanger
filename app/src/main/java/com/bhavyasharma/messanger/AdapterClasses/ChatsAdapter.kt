...
import com.bhavyasharma.messanger.ViewFullImageActivity
...
) : RecyclerView.Adapter<ChatsAdapter.ViewHolder>()
{
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return if(position == 1){
            val view: View = LayoutInflater.from(mContext).inflate(com.bhavyasharma.messanger.R.layout.message_item_right,parent,false)
            ViewHolder(view)
        }
        else {
            val view: View = LayoutInflater.from(mContext).inflate(com.bhavyasharma.messanger.R.layout.message_item_left,parent,false)
            ViewHolder(view)
        }
    }
    ...
    private fun deleteSendMessage(position: Int, holder: ChatsAdapter.ViewHolder)
    {
        val ref = FirebaseDatabase.getInstance().reference.child("Chats").child(mChatList.get(position).getMessageId()!!).removeValue().addOnCompleteListener {task ->
            if(task.isSuccessful) {
                ...
                Toast.makeText(holder.itemView.context,"Error Occured",Toast.LENGTH_SHORT).show()
            }
        }
    }
